import{g as h,t as d,a as m,e as f,w as y,S as b}from"./main-2e79e1a8.js";function g(e){return delete e.otherName,delete e.info,delete e.type,delete e.ingredients,delete e.image,e}const{VITE_APP_SITE:p}={VITE_APP_SITE:"https://two023-dessert-webshop-json-server.onrender.com",VITE_APP_ADMIN_IDENTITY:"528491",BASE_URL:"/2023-Dessert-Webshop-Project/",MODE:"production",DEV:!1,PROD:!0,SSR:!1};function v(){const e=h();if(!e)d("warning","請先登入","login.html");else{const a=JSON.parse(localStorage.getItem("userData")).id;m.get(`${p}/640/user/${a}/carts?_expand=product`,{headers:{authorization:`Bearer ${e}`}}).then(n=>{x(n.data)}).catch(n=>{f(n)})}}v();function x(e){const a=document.querySelector("#cart");let n="";e.length===0?(n=`
        <div class="col-12">
        ...
        `,a.innerHTML=n,w(e),T(e))}function w(e){const a=document.querySelector("#main-content");let n="";e.forEach(t=>n+=`
    ...
    `),a.innerHTML=n,document.querySelector("#confirm").addEventListener("click",t=>{e.some(s=>!s.product.forSale)?y("OOPS","購物車內有完售的商品，請刪除後再重新結帳"):$(t,e)},!1),[...a.children].forEach(t=>{const s=document.querySelector(`[data-num="${t.dataset.num}"] input`).value;S(t,s)})}function S(e,a){e.addEventListener("click",function(n){const{target:o}=n,i=o.closest("li").dataset.num;if(o.closest(".btn")){n.preventDefault();const t=h();if(o.textContent.includes("delete"))m.delete(`${p}/640/carts/${i}`,{headers:{authorization:`Bearer ${t}`}}).then(s=>{d("success","刪除成功"),v()}).catch(s=>{f(s)});else{const s=document.querySelector(`li[data-num="${i}"] input`);if(o.textContent.includes("add"))s.value>9?s.value:s.value++;else if(o.textContent.includes("remove"))s.value<2?s.value:s.value--;else if(o.classList.contains("changeQuantity")){let u=function(c){return isNaN(c)?(d("warning","請輸入阿拉伯數字"),s.value=a,!1):!Number.isInteger(c)||c<=0?(d("warning","請輸入大於零的正整數"),s.value=a,!1):!0};a==s.value?d("question","數量沒變哦 (ㆆᴗㆆ)"):s.value>10?y("數量達上限","如果需要大量訂購，請直接與我們聯絡"):u(+s.value)&&m.get(`${p}/640/carts/${i}`,{headers:{authorization:`Bearer ${t}`}}).then(c=>{let l=c.data;return l={...l,qty:+s.value},m.patch(`${p}/640/carts/${i}`,l,{headers:{authorization:`Bearer ${t}`}})}).then(c=>{d("success","數量修改成功！"),v()}).catch(c=>{f(c)})}}}else return})}function T(e){const a=document.querySelector("#subtotal"),n=document.querySelector("#delivery-fee"),o=document.querySelector("#total");a.textContent=e.reduce((i,t)=>i+Number(t.product.price)*t.qty,0),n.textContent=150,o.textContent=+a.textContent+ +n.textContent}function $(e,a){if(e.target.textContent==="下一步"){const n=document.querySelector("#process-title");e.target.textContent="結　帳",n.textContent="填寫寄送資訊";const o=document.querySelector("#main-content");let i="";i+=`
        ...
        </div>`,o.innerHTML=i,document.querySelector("#useMemberName").addEventListener("change",function(s){if(s.target.checked){const u=JSON.parse(localStorage.getItem("userData")).name;receiver.value=u}else receiver.value=""})}else if(e.target.textContent==="結　帳"){let l=function(r){if((r==null?void 0:r.id)==="delivery-confirm"&&!(r!=null&&r.checked))return d("warning","請詳閱並同意寄送說明"),!1;if(r!=null&&r.value){if((r==null?void 0:r.id)==="phone"&&!/^09(\d){8}/.test(r.value))return d("warning","手機號碼格式不正確"),!1}else return d("warning","請確實填寫所有的欄位"),!1;return!0};const n=document.querySelector('input[name="method"]:checked'),o=document.querySelector('input[name="payment"]:checked'),i=document.querySelector("#receiver"),t=document.querySelector("#phone"),s=document.querySelector("#address"),u=document.querySelector('input[name="shippingTime"]:checked'),c=document.querySelector("#delivery-confirm");l(n)&&l(o)&&l(i)&&l(t)&&l(s)&&l(u)&&l(c)&&function(){const r={receiver:i.value,phone:t.value,address:s.value,payment:o.value,method:n.value,shippingTime:u.value};q(a,r)}()}}function q(e,a){b.fire({icon:"warning",title:"確定送出訂單？",text:"提醒您，按下送出之後即視為交易成立",position:"center",allowOutsideClick:!1,showCancelButton:!0,cancelButtonColor:"#D1741F",cancelButtonText:"再想想看",confirmButtonColor:"#A37A64",confirmButtonText:"送出訂單",showLoaderOnConfirm:!0,preConfirm:async()=>{try{const n=h(),o=document.querySelector("#total").textContent;e=e.map(t=>({...t,product:g(t.product)}));const i={orderNum:new Date().getTime()+`0${e[0].userId}`,content:e,total:Number(o),info:a,createdTime:new Date().getTime(),userId:e[0].userId,isFinished:!1};m.post(`${p}/640/orders`,i,{headers:{authorization:`Bearer ${n}`}}).then(t=>e.forEach(s=>{m.delete(`${p}/640/carts/${s.id}`,{headers:{authorization:`Bearer ${n}`}})}))}catch(n){f(n)}}}).then(n=>{n.isConfirmed&&b.fire({icon:"success",title:"Terima kasih！謝謝您的訂購！",text:"我們將立即為您製作，請耐心等候商品送達",position:"center",confirmButtonColor:"#A37A64",timer:3e3}).then(()=>location.href="member.html")})}
