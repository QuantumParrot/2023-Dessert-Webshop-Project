import{g as b,a as c,e as u,t as i,w as x,S as y}from"./main-24575d4b.js";import{T as S}from"./tab-ee5e9841.js";const{VITE_APP_SITE:d}={VITE_APP_SITE:"https://two023-dessert-webshop-json-server.onrender.com",VITE_APP_ADMIN_IDENTITY:"528491",BASE_URL:"/2023-Dessert-Webshop-Project/",MODE:"production",DEV:!1,PROD:!0,SSR:!1};let g="",f=[];function E(){b()?h():i("warning","請先登入","login.html")}E();window.addEventListener("hashchange",function(){h()});function h(){var s;const t=location.hash.replace("#","")||"orders",a=document.querySelector(`#v-pills-${t}-tab`);a&&new S(a).show(),g=document.querySelector(`#v-pills-${t} #${t}-content`);const e=(s=JSON.parse(localStorage.getItem("userData")))==null?void 0:s.id,r={headers:{authorization:`Bearer ${b()}`}};t==="orders"?c.get(`${d}/600/users/${e}/orders?_sort=id&_order=desc`,r).then(n=>{f=n.data,q(f)}).catch(n=>{u(n)}):t==="collection"?c.get(`${d}/600/users/${e}/collects`,r).then(n=>{f=n.data,T(f)}).catch(n=>{u(n)}):t==="profile"&&c.get(`${d}/600/users/${e}`,r).then(n=>{f=n.data,k()}).catch(n=>{u(n)})}function q(t){let a="";t.length===0?a+=`
    <div class="col-12">
    ...
    `:t.forEach(e=>{a+=`
                    ...
                    <div class="mb-5">
                        ...
                        <div class="row gap-md-5 py-2 border-bottom lh-lg">
                            <div class="col-lg-3 col-12">
                                <p class="text-orange fw-bold">${r.content.name}</p>
                            </div>
                            <div class="col-lg-3 col-12">
                                <p><span class="fw-bold">數量：</span>${r.qty}</p>
                            </div>
                            <div class="col-lg-3 col-12">
                                <p><span class="fw-bold">金額：</span>${r.content.price*r.qty}</p>
                            </div>
                        </div>`).join("")}
                    </div>
        ...
        `}),g.innerHTML=a,$(".accordion-content").hide(),$(".accordion-title").click(function(){$(this).siblings(".accordion-content").slideToggle()})}function T(t){let a="";t.length===0?a+=`
    <div class="col-12">
        <p class="alert bg-tertiary text-center m-0">
        還沒有收藏任何商品哦！去<a href="products.html">逛逛</a>吧！
        </p>
    </div>
    `:t.forEach(({content:s})=>a+=`
    ...
    `,g.innerHTML=a,D(t),B(t)}function D(t){const a=document.querySelector("#profile-form");a.addEventListener("click",function(e){if(e.preventDefault(),e.target.nodeName==="BUTTON"){const r=e.target.dataset.target,s=a.querySelector(`#${r}`);r!=="userPassword"&&e.target.textContent==="修改"?(s.removeAttribute("disabled"),e.target.textContent="送出"):e.target.textContent==="送出"?y.fire({icon:"warning",title:"確定修改資料？",text:`您的${s.name==="name"?"名字":"帳號"}將改為：${s.value}`,showCancelButton:!0,cancelButtonColor:"#D1741F",cancelButtonText:"取消",confirmButtonColor:"#A37A64",confirmButtonText:"確定",showLoaderOnConfirm:!0,preConfirm:async()=>{try{if(s.value===t[s.name]){i("question","資料沒變哦 (ㆆᴗㆆ)");return}const n=b(),o={[s.name]:s.value};e.target.setAttribute("disabled",!0);const m=await c.patch(`${d}/660/users/${t.id}`,o,{headers:{authorization:`Bearer ${n}`}});e.target.removeAttribute("disabled"),i("success","修改完成！"),localStorage.setItem("userData",JSON.stringify(m.data)),h()}catch(n){u(n)}}}).then(n=>{s.setAttribute("disabled",!0),s.value=t[s.name],e.target.textContent="修改"}):r==="userPassword"&&N(t)}})}function N(t){const a=document.querySelector("#change-password-form"),e=a.querySelectorAll("input");a.querySelector('button[type="submit"]').addEventListener("click",function(s){s.preventDefault();const n=e[0].value,o=e[1].value,m=e[2].value;function l(p){const v=/\w{6,}/;if(p)if(v.test(p)){if(n===o){i("warning","新密碼不可與舊密碼相同");return}else if(o!==m){i("warning","兩次密碼不一致");return}}else{i("warning","長度需在六個字以上");return}else{i("warning","欄位不可空白");return}return!0}[...e].every(p=>l(p.value))&&function(){const p={email:t.email,password:n};c.post(`${d}/login/${t.id}`,p).then(v=>{const w={headers:{authorization:`Bearer ${b()}`}};return c.patch(`${d}/660/users/${t.id}`,{password:o},w)}).then(v=>{a.reset(),localStorage.removeItem("token"),localStorage.removeItem("userData"),i("success","修改成功！請重新登入！","login.html")}).catch(v=>{u(v)})}()})}function B(t){const a=document.querySelector("#delivery-form"),e=a.querySelector("#useMemberName"),r=a.querySelector("#receiver"),s=a.querySelector("#phone"),n=a.querySelector("#address");e.addEventListener("change",function(o){o.target.checked?r.value=t.name:r.value=""}),a.addEventListener("submit",function(o){if(o.preventDefault(),s&&!/^09(\d){8}/.test(s.value)){i("warning","手機號碼格式不正確");return}r.value,s.value,n.value,JSON.parse(localStorage.getItem("userData")).id,a.reset()})}
